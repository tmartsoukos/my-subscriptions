...
import { sumAmounts, toCents, fromCents } from "../../money.js";
...
export function buildModel({ subs, todoItems, evItems, finItems, noteItems, courseItems }) {
  ...
  const monthIn = sumAmounts(monthEntries.filter(e => e.kind === "income"), e => e.amount);
  const monthOut = sumAmounts(monthEntries.filter(e => e.kind === "expense"), e => e.amount);
  ...
  const prevIn = sumAmounts(prevEntries.filter(e => e.kind === "income"), e => e.amount);
  const prevOut = sumAmounts(prevEntries.filter(e => e.kind === "expense"), e => e.amount);
  ...
  const byCatCents = {};
  for (const s of active) byCatCents[s.category] = (byCatCents[s.category] || 0) + toCents(monthlyCost(s));
  const donutItems = Object.entries(byCatCents).map(([c, v]) => [c, fromCents(v)])
    .sort((a, b) => b[1] - a[1])
  ...
  const balance = fromCents(toCents(monthIn) - toCents(monthOut) - toCents(monthly));
  ...
  return {
    ...
    balance, prevBalance: fromCents(toCents(prevIn) - toCents(prevOut) - toCents(monthly)),
